...
$where = [];
if ($f_jenis && $f_jenis !== '') {
    $where[] = "mj.jenis_jabatan = '" . mysqli_real_escape_string($koneksi, $f_jenis) . "'";
}
if ($f_eselon) {
    $where[] = "mj.eselon = '" . mysqli_real_escape_string($koneksi, $f_eselon) . "'";
}
...
<head>
    ...
    <title>Cetak Data Jabatan - Dinas Perpustakaan dan Kearsipan Kota Banjarmasin</title>
    <style>
        @media print {
...
<body>
    ...
    <div class="info">
        ...
        <p><strong>Total Data:</strong> <?php echo $total; ?> Jabatan</p>
        <?php if ($f_jenis): ?>
        <p><strong>Filter Jenis:</strong> <?php echo htmlspecialchars($f_jenis); ?></p>
        <?php endif; ?>
        <?php if ($f_eselon): ?>
        <p><strong>Filter Eselon:</strong> <?php echo htmlspecialchars($f_eselon); ?></p>
        <?php endif; ?>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="18%">Jenis Jabatan</th>
                <th width="10%">Jumlah Pegawai</th>
        ...
        <tbody>
            ...
            if ($total > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo '<td class="text-center">' . htmlspecialchars($row['eselon'] ?? '-') . '</td>';
                    echo '<td class="text-center">' . htmlspecialchars($row['jenis_jabatan']) . '</td>';
