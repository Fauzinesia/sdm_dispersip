...
$f_jabatan = $_GET['f_jabatan'] ?? '';
$f_pangkat = $_GET['f_pangkat'] ?? '';
$f_kepegawaian = $_GET['f_kepegawaian'] ?? '';

// Build WHERE clause
$where = [];
if ($f_status && $f_status !== '') {
    $where[] = "p.status_aktif = '" . mysqli_real_escape_string($koneksi, $f_status) . "'";
}
if ($f_jabatan) {
    $where[] = "p.jabatan_id = '" . mysqli_real_escape_string($koneksi, $f_jabatan) . "'";
}
if ($f_pangkat) {
    $where[] = "p.pangkat_id = '" . mysqli_real_escape_string($koneksi, $f_pangkat) . "'";
}
if ($f_kepegawaian && $f_kepegawaian !== '') {
    $where[] = "p.status_kepegawaian = '" . mysqli_real_escape_string($koneksi, $f_kepegawaian) . "'";
}
...
<head>
    ...
    <title>Cetak Data Pegawai - Dinas Perpustakaan dan Kearsipan Kota Banjarmasin</title>
    <style>
        @media print {
...
<body>
    ...
    <div class="info">
        ...
        <p><strong>Total Data:</strong> <?php echo $total; ?> Pegawai</p>
        <?php if ($f_status): ?>
        <p><strong>Filter Status:</strong> <?php echo htmlspecialchars($f_status); ?></p>
        <?php endif; ?>
        <?php if ($f_kepegawaian): ?>
        <p><strong>Filter Status Kepegawaian:</strong> <?php echo htmlspecialchars($f_kepegawaian); ?></p>
        <?php endif; ?>
    </div>
        ...
        <tbody>
            ...
            if ($total > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    $pangkat_gol = $row['nama_pangkat'] 
                        ? htmlspecialchars($row['nama_pangkat']) . ' (' . htmlspecialchars($row['golongan']) . ')'
                        : '-';
                    ...
                    echo '<td class="text-center">' . htmlspecialchars($row['jk']) . '</td>';
                    echo '<td>' . htmlspecialchars($row['nama_jabatan'] ?? '-') . '</td>';
                    echo '<td>' . $pangkat_gol . '</td>';
                    echo '<td>' . htmlspecialchars($row['nik']) . '</td>';
                    echo '<td class="text-center">' . htmlspecialchars($row['status_kepegawaian']) . '</td>';
                    echo '<td class="text-center">' . htmlspecialchars($row['status_aktif']) . '</td>';
                    echo '</tr>';
                }
            } else {
