  ...
  <div class="page-content">
    <div class="container-xxl">
      ...
      <div class="row">
        <div class="col-12">
          <div class="page-title-box">
            <h4 class="page-title">Penilaian Kinerja (Verifikator)</h4>
          </div>
        </div>
      </div>
      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          <form method="get" action="verifikator/penilaian_kinerja.php" class="row g-3">
            ...
            <div class="col-md-6 d-flex align-items-end gap-2"><button type="submit" class="btn btn-primary"><i class="ti ti-filter me-1"></i>Terapkan Filter</button><a href="verifikator/penilaian_kinerja.php" class="btn btn-light">Reset</a></div>
          </form>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-header border-bottom border-dashed d-flex justify-content-between align-items-center">
          <div>
            ...
            <a href="admin/penilaian_kinerja/tambah.php" class="btn btn-primary ms-2"><i class="ti ti-plus me-1"></i>Tambah Data</a>
          </div>
        </div>
        <div class="card-body">
          <div class="table-responsive-sm">
            <table class="table table-striped mb-0">
              <thead>
                <tr>
                  <th>No</th>
                  <th><a class="link-reset" href="verifikator/penilaian_kinerja.php?sort=nama_lengkap&order=<?php echo ($sort==='nama_lengkap' && $order==='ASC')?'desc':'asc'; ?>">Nama Pegawai<?php echo $sort==='nama_lengkap' ? ($order==='ASC'?' ▲':' ▼') : ''; ?></a></th>
                  <th><a class="link-reset" href="verifikator/penilaian_kinerja.php?sort=periode&order=<?php echo ($sort==='periode' && $order==='ASC')?'desc':'asc'; ?>">Periode<?php echo $sort==='periode' ? ($order==='ASC'?' ▲':' ▼') : ''; ?></a></th>
                  <th><a class="link-reset" href="verifikator/penilaian_kinerja.php?sort=nilai_kuantitas&order=<?php echo ($sort==='nilai_kuantitas' && $order==='ASC')?'desc':'asc'; ?>">Kuantitas<?php echo $sort==='nilai_kuantitas' ? ($order==='ASC'?' ▲':' ▼') : ''; ?></a></th>
                  <th><a class="link-reset" href="verifikator/penilaian_kinerja.php?sort=nilai_kualitas&order=<?php echo ($sort==='nilai_kualitas' && $order==='ASC')?'desc':'asc'; ?>">Kualitas<?php echo $sort==='nilai_kualitas' ? ($order==='ASC'?' ▲':' ▼') : ''; ?></a></th>
                  <th><a class="link-reset" href="verifikator/penilaian_kinerja.php?sort=nilai_perilaku&order=<?php echo ($sort==='nilai_perilaku' && $order==='ASC')?'desc':'asc'; ?>">Perilaku<?php echo $sort==='nilai_perilaku' ? ($order==='ASC'?' ▲':' ▼') : ''; ?></a></th>
                  <th><a class="link-reset" href="verifikator/penilaian_kinerja.php?sort=skor_akhir&order=<?php echo ($sort==='skor_akhir' && $order==='ASC')?'desc':'asc'; ?>">Skor Akhir<?php echo $sort==='skor_akhir' ? ($order==='ASC'?' ▲':' ▼') : ''; ?></a></th>
                  <th>Predikat</th>
                  <th>Penilai</th>
                  <th><a class="link-reset" href="verifikator/penilaian_kinerja.php?sort=created_at&order=<?php echo ($sort==='created_at' && $order==='ASC')?'desc':'asc'; ?>">Dibuat<?php echo $sort==='created_at' ? ($order==='ASC'?' ▲':' ▼') : ''; ?></a></th>
                  <th>Aksi</th>
                </tr>
              </thead>
              <tbody>
                ...
                if ($res && mysqli_num_rows($res) > 0) {
                  ...
                  while ($row = mysqli_fetch_assoc($res)) {
                    echo '<tr>'
                      ...
                      . '<td>'
                        . '<a href="admin/penilaian_kinerja/detail.php?id=' . htmlspecialchars($row['penilaian_id']) . '" class="btn btn-sm btn-info me-1" title="Detail"><i class="ti ti-eye"></i></a>'
                        . '<a href="admin/penilaian_kinerja/edit.php?id=' . htmlspecialchars($row['penilaian_id']) . '" class="btn btn-sm btn-warning me-1" title="Edit"><i class="ti ti-pencil"></i></a>'
                        . '<a href="admin/penilaian_kinerja/hapus.php?id=' . htmlspecialchars($row['penilaian_id']) . '" class="btn btn-sm btn-danger" onclick="return confirm(\'Yakin ingin menghapus data ini?\')" title="Hapus"><i class="ti ti-trash"></i></a>'
                      . '</td>'
                      . '</tr>';
                  }
                } else {
          ...
          if ($totalPages > 1):
            $baseQS = 'verifikator/penilaian_kinerja.php?sort=' . urlencode($sort) . '&order=' . strtolower($order)
              . '&f_year=' . urlencode($f_year)
              . '&f_month=' . urlencode($f_month)
              . '&f_penilai=' . urlencode((string)$f_penilai)
              . '&f_min=' . urlencode($f_min)
